...
test('Piping from a ReadableStream in readable state which becomes closed after pipeTo call to a WritableStream in writable state', t => {
...
test('Piping from a ReadableStream in readable state which becomes errored after pipeTo call to a WritableStream in writable state', t => {
  ...
  errorReadableStream('Bad thing');
});

test('Piping from a ReadableStream in waiting state which becomes readable after pipeTo call to a WritableStream in writable state', t => {
  var enqueue;
  var pullCount = 0;
  var rs = new ReadableStream({
    start(enqueue_) {
...
test('Piping from a ReadableStream in waiting state which becomes errored after pipeTo call to a WritableStream in writable state', t => {
  var errorReadableStream;
  var pullCount = 0;
  var rs = new ReadableStream({
    start(enqueue, close, error) {
...
test('Piping from a ReadableStream in waiting state to a WritableStream in writable state which becomes errored after pipeTo call', t => {
  var writeCalled = false;
...
test('Piping from a ReadableStream in readable state to a WritableStream in waiting state which becomes writable after pipeTo call', t => {
  var enqueue;
  var pullCount = 0;
  var rs = new ReadableStream({
    start(enqueue) {
...
test('Piping from a ReadableStream in readable state to a WritableStream in waiting state which becomes errored after pipeTo call', t => {
  var writeCalled = false;
...
test('Piping from a ReadableStream in readable state which becomes errored after pipeTo call to a WritableStream in waiting state', t => {
  var errorReadableStream;
  var rs = new ReadableStream({
    start(enqueue, close, error) {
      enqueue("World");
...
test('Piping from a ReadableStream in waiting state to a WritableStream in waiting state where both become ready after pipeTo', t => {
  var enqueue;
  var pullCount = 0;
  var rs = new ReadableStream({
    start(enqueue_) {
  ...
  // Check that nothing happens before calling done(), and then call done() to
  // check that pipeTo is woken up.
  setTimeout(
      () => {
        t.equal(pullCount, 1);
...
test('Piping from a ReadableStream in waiting state to a WritableStream in waiting state which becomes writable after pipeTo call', t => {
  var pullCount = 0;
  var rs = new ReadableStream({
    pull() {
      ++pullCount;
...
test('Piping from a ReadableStream in waiting state which becomes closed after pipeTo call to a WritableStream in waiting state', t => {
  var closeReadableStream;
  var pullCount = 0;
  var rs = new ReadableStream({
    start(enqueue, close) {
...
test('Piping from a ReadableStream in waiting state which becomes errored after pipeTo call to a WritableStream in waiting state', t => {
  var errorReadableStream;
  var pullCount = 0;
  var rs = new ReadableStream({
    start(enqueue, close, error) {
